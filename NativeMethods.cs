...
public static class NativeMethods
{
  ...
  public static bool StartStressTest(int threadCount, string mode = "heavy", int? coreIndex = null)
  {
    if (threadCount <= 0)
    {
    ...
    var result = Stress.Start(threadCount, mode, coreIndex);
  ...
  public static double RunSingleCoreBenchmark(int? coreIndex = null) => Benchmark.RunSingleThread(coreIndex);
  ...
  private static UIntPtr ComputeAffinityMask(int coreIndex)
  {
    if (coreIndex < 0)
    {
      return UIntPtr.Zero;
    ...
    int maxIndex = Math.Max(Environment.ProcessorCount - 1, 0);
    maxIndex = Math.Min(maxIndex, pointerBits - 1);
    int clampedIndex = Math.Min(coreIndex, maxIndex);
    ...
    return new UIntPtr(1UL << clampedIndex);
  }
  ...
  private sealed class StressEngine
  {
    ...
    private UIntPtr workerAffinityMask = UIntPtr.Zero;

    public bool Start(int threadCount, string mode = "heavy", int? coreIndex = null)
    {
      lock (sync)
      {
        if (cts != null)
        ...
        stressMode = mode;
        workers.Clear();
        workerAffinityMask = UIntPtr.Zero;
        if (threadCount == 1 && coreIndex.HasValue && coreIndex.Value >= 0)
        {
          workerAffinityMask = ComputeAffinityMask(coreIndex.Value);
        }
    ...
    {
      if (workerAffinityMask != UIntPtr.Zero && threadId == 0)
      {
        SetThreadAffinityMask(GetCurrentThread(), workerAffinityMask);
      }
    ...
    private unsafe void WorkerLoopHeavy(CancellationToken token)
    {
      // Small hot buffers (L1/L2 sized)
      const int Size = 4096; // 32 KB doubles
      double[] data = new double[Size];

      for (int i = 0; i < Size; i++)
        data[i] = (i + 1) * 0.0000001;

      double scalar = 1.0000000001;
      ...
      try
      {
        fixed (double* p = data)
        {
          while (!token.IsCancellationRequested)
          {
            ...
            {
              Vector256<double> v0 = Vector256.Create(1.0000001);
              Vector256<double> v1 = Vector256.Create(1.0000002);
              Vector256<double> v2 = Vector256.Create(1.0000003);
              Vector256<double> v3 = Vector256.Create(1.0000004);

              for (int iter = 0; iter < 4096; iter++)
              {
                // Tight dependency chain (power-heavy)
                v0 = Fma.MultiplyAdd(v0, v1, v2);
                v1 = Fma.MultiplyAdd(v1, v2, v3);
                v2 = Fma.MultiplyAdd(v2, v3, v0);
                v3 = Fma.MultiplyAdd(v3, v0, v1);

                // Force load/store traffic
                int idx = (iter * 4) & (Size - 4);
                Vector256<double> mem = Avx.LoadVector256(p + idx);
                mem = Fma.MultiplyAdd(mem, v0, v1);
                Avx.Store(p + idx, mem);
              }
            }
            else
            {
              // Scalar fallback (still power-dense)
              double a = 1.0000001;
              double b = 1.0000002;
              double c = 1.0000003;
              double d = 1.0000004;
              ...
              for (int iter = 0; iter < 8192; iter++)
              {
                ...
                int idx = iter & (Size - 1);
                data[idx] = (data[idx] * a) + b;
                scalar += data[idx];
              }
            }

            // Prevent optimization & keep values bounded
            if (scalar > 1e100 || scalar < -1e100)
              scalar = 1.0000001;
          }
        }
      }
  ...
  private sealed class BenchmarkEngine
  {
    ...
    private const double NormalizationFactorMulti = 101_000d;  // Recalibrated for refactored benchmark algorithm
    private const int BatchSize = 1024;

    public double RunSingleThread(int? coreIndex = null)
    {
      return ExecuteBenchmark(1, TargetSecondsSingle, NormalizationFactorSingle, coreIndex);
    }

    public double RunMultiThread()
    {
      int threadCount = Math.Max(1, Environment.ProcessorCount);
      return ExecuteBenchmark(threadCount, TargetSecondsMulti, NormalizationFactorMulti, null);
    }

    [System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.AggressiveOptimization)]
    private static double ExecuteBenchmark(int threadCount, double durationSeconds, double normalizationFactor, int? coreIndex = null)
    {
      long[] results = new long[threadCount];
      UIntPtr benchmarkAffinityMask = UIntPtr.Zero;
      if (threadCount == 1 && coreIndex.HasValue && coreIndex.Value >= 0)
      {
        benchmarkAffinityMask = ComputeAffinityMask(coreIndex.Value);
      }
      ...
      for (int t = 0; t < threadCount; t++)
      {
        ...
        {
          // Pin to specific core if affinity mask is set
          if (benchmarkAffinityMask != UIntPtr.Zero && tid == 0)
          {
            SetThreadAffinityMask(GetCurrentThread(), benchmarkAffinityMask);
          }
          ...
          while (true)
          {
            ...
            {
              // Math-heavy independent work (optimized for Zen 5 compatibility)
              f = Math.Sqrt(f * 0.999999d + 0.000001d);
              
              // Use combined SinCos operation instead of separate Sin/Cos calls
              (double sin, double cos) = Math.SinCos(f);
              f = sin + cos;
