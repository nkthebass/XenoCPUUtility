...
namespace CPUUtilityHybrid
{
    public class Form1 : Form
    {
        ...
        private async void Form1_Load(object? sender, EventArgs e)
        {
            ...
            // Extract www folder from embedded resources
            var wwwFolder = Path.Combine(Path.GetTempPath(), "XenoCPUUtility_www");
            ExtractEmbeddedResources(wwwFolder);
            ...
            var indexPath = Path.Combine(wwwFolder, "index.html");
        ...
        private void ExtractEmbeddedResources(string targetFolder)
        {
            ...
            var resourceNames = assembly.GetManifestResourceNames()
                .Where(r => r.StartsWith("CPUUtilityHybrid.www."))
                .ToList();

            foreach (var resourceName in resourceNames)
            {
                // Extract relative path: "CPUUtilityHybrid.www.index.html" -> "index.html"
                var relativePath = resourceName.Substring("CPUUtilityHybrid.www.".Length);
                var filePath = Path.Combine(targetFolder, relativePath);
        ...
        private void OnWebMessageReceived(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
        {
            try
            {
                ...
                switch (cmd)
                {
                    case "startStress":
                        {
                            ...
                            string mode = args.TryGetProperty("mode", out var m) ? m.GetString() ?? "heavy" : "heavy";
                            ...
                            var coreIdValue = args.TryGetProperty("coreId", out var coreArg) ? coreArg.GetInt32() : -1;
                            int? coreTarget = coreIdValue >= 0 && threads == 1 ? coreIdValue : null;
                            try
                            {
                                ...
                                var messageText = success
                                    ? $"{modeLabel} stress test started. {ramMessage}"
                    ...
                    case "runPathTracerBenchmark":
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    if (args.TryGetProperty("numCores", out JsonElement coresElement))
                                    {
                                        int requested = coresElement.GetInt32();
                                        if (requested > 0 && requested <= Environment.ProcessorCount)
                                        {
                                            numCores = requested;
                                        }
                                    }
                            ...
                            System.Windows.Forms.Form displayForm = new System.Windows.Forms.Form
                            {
                                Text = "Path Tracer Rendering...",
                                Width = 768 + 20,
                                Height = 768 + 40,
                            ...
                            Task.Run(() =>
                            {
                                try
                                {
                                    var pathTracer = new PathTracerBenchmark();
                                    var result = pathTracer.XenoCPUUtilityRunBenchmark(samplesPerPixel: samplesPerPixel, imageWidth: 768, imageHeight: 768, durationSeconds: 0, numThreads: numCores, displayForm: displayForm, pictureBox: pictureBox);
                    ...
                    case "runMultiCoreBenchmark":
                        {
                            Task.Run(() =>
                            {
                                ...
                                // Throwaway warmup benchmark
                                try
                                {
                                    var warmupStopwatch = Stopwatch.StartNew();
                                    TimeSpan warmupLastBroadcast = TimeSpan.Zero;
                                    const double warmupDuration = 2.0;

                                    while (warmupStopwatch.Elapsed.TotalSeconds < warmupDuration)
                                    {
                                        NativeMethods.RunMultiCoreBenchmark();

                                        // Broadcast warmup progress
                                        if (warmupStopwatch.Elapsed - warmupLastBroadcast >= TimeSpan.FromMilliseconds(200))
                                        {
                                            double warmupElapsed = Math.Min(warmupStopwatch.Elapsed.TotalSeconds, warmupDuration);
                                            BeginInvoke(new Action(() =>
                                            {
                                                Broadcast("benchmarkProgress", new
                                                {
                                                    elapsedSeconds = warmupElapsed,
                                                    targetSeconds = warmupDuration,
                                                    sampleCount = 0,
                                                    warmup = true
                                                });
                                            }));
                                            warmupLastBroadcast = warmupStopwatch.Elapsed;
                                        }
                                    }

                                    // Broadcast warmup completion
                                    BeginInvoke(new Action(() =>
                                    {
                                        Broadcast("benchmarkProgress", new
                                        {
                                            elapsedSeconds = warmupDuration,
                                            targetSeconds = warmupDuration,
                                            sampleCount = 0,
                                            warmup = true,
                                            completed = true
                                ...
                                try
                                {
                                    ...
                                    while (stopwatch.Elapsed.TotalSeconds < durationSeconds)
                                    {
                                        ...
                                        if (sampleScore > 0)
                                        {
                                            ...
                                            runSamples.Add(sampleScore);
                                        }

                                        double elapsedInCurrentRun = (stopwatch.Elapsed.TotalSeconds % secondsPerRun);
                                        int runAtThisTime = Math.Min((int)Math.Ceiling(stopwatch.Elapsed.TotalSeconds / secondsPerRun), runCount);

                                        if (runAtThisTime > currentRun && runSamples.Count > 0)
                                        {
                                            double runAverage = runSamples.Average();
                                            runScores.Add(runAverage);
                                            runSamples.Clear();
                                            currentRun = runAtThisTime;
                                        }
                                    ...
                                    double averageScore = runScores.Count > 0 ? runScores.Average() : 0.0;
                                    ...
                                    BeginInvoke(new Action(() =>
                                    {
                                        Reply(id, cmd, new
                                        {
                                            ...
                                            individualScores = runScores.ToArray()
                                        });
                                    }));
                                }
                                catch (Exception ex)
                    ...
                    case "runGPUBenchmarkDirectX":
                        {
                            Task.Run(() =>
                            {
                                ...
                                try
                                {
                                    if (args.ValueKind != JsonValueKind.Undefined && args.ValueKind != JsonValueKind.Null)
                                    {
                                        if (args.TryGetProperty("width", out JsonElement wArg))
                                            width = wArg.GetInt32();
                                        if (args.TryGetProperty("height", out JsonElement hArg))
                                            height = hArg.GetInt32();
                                        if (args.TryGetProperty("samplesPerPixel", out JsonElement sampArg))
                                            samplesPerPixel = sampArg.GetInt32();
                                        if (args.TryGetProperty("maxBounces", out JsonElement bounceArg))
                                            maxBounces = bounceArg.GetInt32();
                                    }
                                }
                                catch
                                {
                                ...
                                try
                                {
                                    // Call GPU DirectX benchmark
                                    string imageBase64 = NativeMethods.RunGPUBenchmarkDirectX(width, height, samplesPerPixel, maxBounces);
                                    ...
                                    BeginInvoke(new Action(() =>
                                    {
                                        Reply(id, cmd, new
                                        {
                                            ...
                                            success = !string.IsNullOrEmpty(imageBase64)
                                        });
                                    }));
                                }
                                catch (Exception ex)
                    ...
                    case "runSingleCoreBenchmark":
                        {
                            try
                            {
                                var score = NativeMethods.RunSingleCoreBenchmark();
                                Reply(id, cmd, new { success = true, score = score });
                            }
                            catch (Exception ex)
                            {
                                Reply(id, cmd, new { success = false, error = ex.Message });
                            }
                        }
                        break;
                    ...
                    case "getGPUModel":
                        {
                            try
                            {
                                ...
                                Reply(id, cmd, new { success = true, gpuModel = gpuModel });
                            }
                            catch (Exception ex)
                            {
                                Reply(id, cmd, new { success = false, error = ex.Message });
        ...
        private int GetWindowsVersion()
        {
            try
            {
                ...
                {
                    return osVersion.Build >= 22000 ? 11 : 10;
                }
        ...
        private long GetRamAmount()
        {
            try
            {
                ...
                foreach (var obj in results.Cast<ManagementObject>())
                {
                    if (obj["TotalPhysicalMemory"] != null && long.TryParse(obj["TotalPhysicalMemory"].ToString(), out var ramBytes))
                    {
                        return ramBytes / (1024 * 1024 * 1024); // Convert to GB
                    }
                }
            }
            catch { }
            ...
            try
            {
                var ramStatus = new System.Diagnostics.ProcessStartInfo("wmic")
                {
                ...
                using (var process = System.Diagnostics.Process.Start(ramStatus))
                {
                    if (process?.StandardOutput != null)
                    {
                        string output = process.StandardOutput.ReadToEnd();
                        string[] lines = output.Split('\n');
                        if (lines.Length > 1 && long.TryParse(lines[1].Trim(), out var memKB))
                        {
                            return memKB / (1024 * 1024); // Convert from KB to GB
                        }
                    }
                }
            }
