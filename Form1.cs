                    ...
                    case "startStress":
                        {
                            ...
                            try
                            {
                                bool success = NativeMethods.StartStressTest(threads, mode, coreTarget);
                    ...
                    case "runSingleCoreBenchmark":
                        {
                            Task.Run(() =>
                            {
                                ...
                                try
                                {
                                    if (args.ValueKind != JsonValueKind.Undefined && args.ValueKind != JsonValueKind.Null)
                                    {
                                        if (args.TryGetProperty("coreId", out JsonElement coreArg))
                                        {
                                            int coreId = coreArg.GetInt32();
                                            if (coreId >= 0)
                                            {
                                                coreIndex = coreId;
                                            }
                                ...
                                {
                                    // Keep coreIndex = null if parsing fails
                                }

                                try
                                {
                                    double score = NativeMethods.RunSingleCoreBenchmark(coreIndex);
                                    BeginInvoke(new Action(() =>
                                    {
                                        Reply(id, cmd, new
                                        {
