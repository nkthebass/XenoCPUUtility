...
namespace CPUUtilityHybrid
{
    public class PathTracerBenchmark
    {
        ...
        private HitRecord XenoCPUUtilityTraceRay(Ray ray, float maxDist = float.MaxValue)
        {
        ...
        private Vector3 XenoCPUUtilityTracePathRecursive(Ray ray, int depth)
        {
            if (depth > 4) return Vector3.Zero;

            HitRecord hit = XenoCPUUtilityTraceRay(ray);
            ...
            if (hit.Material == MaterialType.Glass)
            {
                ...
                if (hit.Roughness < 0.2f)  // Only for clear glass
                {
                    ...
                    result += hit.Color * XenoCPUUtilityTracePathRecursive(refractRay, depth + 1) * 0.98f;
                }
                else
                {
                    ...
                    result += hit.Color * XenoCPUUtilityTracePathRecursive(scatterRay, depth + 1) * 0.9f;
                }
            }
            else if (hit.Material == MaterialType.Metal)
            {
                ...
                result += hit.Color * XenoCPUUtilityTracePathRecursive(reflectRay, depth + 1) * 0.95f;
            }
            else  // Diffuse material
            {
                ...
                foreach (var sphere in spheres)
                {
                    if (sphere.IsLight)
                    {
                        ...
                        if (cosTheta > 0)
                        {
                            ...
                            HitRecord shadowHit = XenoCPUUtilityTraceRay(shadowRay, Vector3.Distance(hit.Point, sphere.Center));
                ...
                if (depth < 3)
                {
                    ...
                    result += hit.Color * XenoCPUUtilityTracePathRecursive(bounceRay, depth + 1) * 0.25f;
                }
            }
                    ...
                    for (int y = startY; y < endY; y++)
                    {
                        for (int x = startX; x < endX; x++)
                        {
                            ...
                            for (int s = 0; s < samplesPerPixel; s++)
                            {
                                ...
                                color += XenoCPUUtilityTracePathRecursive(ray, 0);
                                lock (this)
                                {
                                    totalSamples++;
                                }
